...
void gps_testTask(void *pData)
{
    ...
    while(1)
    {
        if(isGpsOn)
        {
            ...
            //snprintf(body,sizeof(buffer2),"id=%s&time=%d&lat=%f&lon=%f&speed=%f&bearing=%.1f&alt=%f&acc=%.1f&batt=%.1f",
            //                                        DEV_ID,time(NULL),latitude,longitude,isFixed*1.0,0.0,gpsInfo->gga.altitude,0.0,percent*1.0);
            // Only needed values
            snprintf(body,sizeof(buffer2),"id=%s&time=%d&lat=%f&lon=%f&alt=%f&batt=%.1f",
                                                    DEV_ID,time(NULL),latitude,longitude,gpsInfo->gga.altitude,percent*1.0);
            uint8_t status;
            Network_GetActiveStatus(&status);
            if(status)
            {
                GPIO_Set(UPLOAD_DATA_LED,GPIO_LEVEL_HIGH);
                if(Http_Post(SERVER_IP,SERVER_PORT,"/",body,strlen(body),buffer,sizeof(buffer)) <0 )
